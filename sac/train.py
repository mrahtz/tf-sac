...
def train_sac(buffer: ReplayBuffer, model: SACModel, train_env, test_env, n_start_env_steps, log_every_n_steps,
              checkpoint_every_n_steps, train_n_steps, test_every_n_steps, batch_size, render):
    ...
    while n_steps < train_n_steps:
        ...
        if n_steps % test_every_n_steps == 0:
            run_test_episodes(model, test_env, n_episodes=10, render=render)
...
def run_test_env(model, model_load_dir, render, env_id, seed, log_dir):
    env = make_env(env_id, seed, log_dir, 'test_async', record_videos=True)
    while True:
        model.load(model_load_dir)
        run_test_episodes(model, env, n_episodes=1, render=render)
